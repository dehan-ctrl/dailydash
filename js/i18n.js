...
const EXPLAIN_RULES = [
  [/Only (\d+)\/(\d+) fully-logged days and (\d+) weigh-ins \(need 4 and 3\)\. Not enough data to coach honestly — targets held; log more this week\./,
    'Yalnızca $1/$2 tam kayıtlı gün ve $3 tartı var (4 ve 3 gerekli). Dürüst koçluk için veri yetersiz — hedefler korundu; bu hafta daha çok kayıt tutun.'],
  [/You said this period was not fully tracked — nothing learned, targets held\. Track everything you eat and check in again\./,
    'Bu dönemin tamamen kaydedilmediğini söylediniz — hiçbir şey öğrenilmedi, hedefler korundu. Yediğiniz her şeyi kaydedin ve tekrar kontrol yapın.'],
  ...
  [/Trend ([+-]?[\d.]+) kg over (\d+) days \(([+-]?[\d.]+) kg\/week\) vs target ([+-]?[\d.]+); average intake (\d+) kcal\/day; estimated TDEE (\d+) kcal\./,
    '$2 günde trend $1 kg (haftada $3 kg), hedef $4; ortalama alım günde $5 kcal; tahmini TDEE $6 kcal.'],
...
export const TR = {
  ...
  '✓ Checked in today': '✓ Bugün kontrol yapıldı',
  'Check-in unlocks tomorrow': 'Kontrol yarın açılır',
  'Check-in unlocks in {n} days': 'Kontrol {n} gün sonra açılır',
  'Early check-in available': 'Erken kontrol hazır', 'Early check-in': 'Erken kontrol',
  'Did you track everything you ate this period?': 'Bu dönemde yediğiniz her şeyi kaydettiniz mi?',
  'Be honest — the coach only learns from fully tracked days.': 'Dürüst olun — koç yalnızca tam kayıtlı günlerden öğrenir.',
