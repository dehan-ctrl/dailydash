...
// sheet = {meal, tab, q, results, picked:{food, servingIdx, qty}|null, editing, recipeDraft, busy}
...
function wire(log) {
  ...
  root.querySelectorAll('[data-add]').forEach((b) =>
    (b.onclick = () => { sheet = { meal: +b.dataset.add, tab: 'search', q: '', results: [], picked: null, editing: false, recipeDraft: null }; render(); }));
...
function detailView() {
  ...
  <div class="macrogrid">
    <div><b>${m.kcal}</b><span>Cal</span></div><div><b>${m.p}</b><span>Protein</span></div>
    <div><b>${m.c}</b><span>Carbs</span></div><div><b>${m.f}</b><span>Fat</span></div>
  </div>
...
function editView(food, servings) {
  ...
  <div class="row"><input id="ek" type="number" step="0.1" value="${food.per100g.kcal}" placeholder="kcal">
    <input id="ep" type="number" step="0.1" value="${food.per100g.p}" placeholder="protein"></div>
  <div class="row"><input id="ec" type="number" step="0.1" value="${food.per100g.c}" placeholder="carbs">
    <input id="ef" type="number" step="0.1" value="${food.per100g.f}" placeholder="fat"></div>
...
async function renderSheet() {
  ...
  if (sheet.picked) body = detailView();
    ...
    body = `<div class="row"><input id="q" placeholder="Search foods…" value="${sheet.q}" style="flex:2 1 120px">
      <button class="ghost" id="go">Search</button>
...
function wireSheet(el) {
  ...
  if (go) {
    const run = async () => {
      ...
      try {
        ...
        sheet.results = await Promise.all([...off, ...usda].map(async (f) =>
  ...
  /* detail */
  const pback = q('#pback');
  if (pback) pback.onclick = () => { sheet.picked = null; renderSheet(); };
  el.querySelectorAll('[data-serv]').forEach((b) =>
    (b.onclick = () => { sheet.picked.servingIdx = +b.dataset.serv; sheet.picked.qty = +q('#pqty').value || 1; renderSheet(); }));
  const pqty = q('#pqty');
  if (pqty && !sheet.editing) pqty.onchange = () => { sheet.picked.qty = +pqty.value || 0; renderSheet(); };
  const pedit = q('#pedit');
  if (pedit) pedit.onclick = () => { sheet.editing = true; renderSheet(); };
  ...
  if (peditback) peditback.onclick = () => { sheet.editing = false; renderSheet(); };
  const nsadd = q('#nsadd');
